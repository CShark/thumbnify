...
namespace Thumbnify.NAudio {
    public class NAudioEngine : INotifyPropertyChanged, ISpectrumPlayer, IWaveformPlayer, IDisposable {
        ...
        private int waveformCompressedPointCount = 12000;
        ...
        private void GenerateWaveformData(string path) {
            ...
            if (File.Exists(path + ".peaks")) {
                using (var file = new FileStream(path + ".peaks", FileMode.Open, FileAccess.Read)) {
                    using (var reader = new BinaryReader(file)) {
                        ...
                        for (int i = 0; i < count; i++) {
                            WaveformData[i] = reader.ReadSingle();
                        }
        ...
        private void waveformGenerateWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e) {
            if (e.Cancelled) {
                ...
                using (var file = new FileStream(pendingWaveformPath + ".peaks", FileMode.Create,
                           FileAccess.ReadWrite)) {
                    using (var writer = new BinaryWriter(file)) {
                        writer.Write(WaveformData.LongLength);

                        for (int i = 0; i < WaveformData.LongLength; i++) {
                            writer.Write(WaveformData[i]);
                        }
                    }
                }
            }
        ...
        private void waveformGenerateWorker_DoWork(object sender, DoWorkEventArgs e) {
            ...
            while (currentPointIndex * 2 < waveformParams.Points) {
                ...
                var leftMax = (float)Math.Sqrt(waveformAggregator.LeftMaxVolume);
                var rightMax = (float)Math.Sqrt(waveformAggregator.RightMaxVolume);

                waveformData.Add(leftMax);
                waveformData.Add(rightMax);

                if (leftMax > maxLeftPointLevel)
                    maxLeftPointLevel = leftMax;
                if (rightMax > maxRightPointLevel)
                    maxRightPointLevel = rightMax;
        ...
        public void UpdateChannelPosition() {
            if (ActiveStream != null) {
                ChannelPosition = (double)ActiveStream.Position / ActiveStream.Length *
        ...
        private void inputStream_Sample(object sender, SampleEventArgs e) {
            ...
            long repeatStartPosition =
                (long)(SelectionBegin.TotalSeconds / ActiveStream.TotalTime.TotalSeconds * ActiveStream.Length);
            long repeatStopPosition =
                (long)(SelectionEnd.TotalSeconds / ActiveStream.TotalTime.TotalSeconds * ActiveStream.Length);
